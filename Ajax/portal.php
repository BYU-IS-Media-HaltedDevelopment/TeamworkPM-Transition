...
require_once('Dashboard_dataRetrieval.lib.php');
...
$ret_json .= "\"request\": {\"status\":\"ready\",\"json_throttler\":".
		json_encode(throttler($max_requests_per_second,true,true)).
		"}";
if(isset($_POST) && !is_null($_POST)) {
	...
	switch(strtolower($_POST['method'])) {
		...
		case "put":
			...
			if($throttle_obj = throttler($max_requests_per_second,true,true)) {
				$ret_json .= ",\"response\":".
					json_encode(putTeamworkPMData($_POST['data'],$_POST['action'],$_POST['id'],"json"));
			} 
			else {
				$ret_json .=  "\"request\":{\"status\":\"not ready\"}";
			}
